...
class Professor():
    def __init__(self,num,url):
        ...
        if url == '':
            self.url = 'http://www.cert.org.cn/publish/main/11/index_'+str(num)+'.html'
        else:
            self.url = url
        self.ua = get_ua()
        self.headers = { "Accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8",
                     "Accept-Encoding":"gzip, deflate, sdch",
                     "Accept-Language":"zh-CN,zh;q=0.8",
                     "Referer":"http://www.cert.org.cn/publish/main/11/index.html",
                     "User-Agent":self.ua,
                     "Host":"www.cert.org.cn",
                     "Connection":"keep-alive",
                     "Upgrade-Insecure-Requests":"1",
...
if __name__=="__main__":
    ...
    for i in range(1,pagenum+1):
        ...
        if i == 1:
            p = Professor(i,'http://www.cert.org.cn/publish/main/11/index.html')
            p.get_url_list()
        else:
            p = Professor(i,'')
            p.get_url_list()
